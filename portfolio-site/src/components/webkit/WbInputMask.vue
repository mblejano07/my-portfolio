...
<template>
  <div :class="`flex w-full flex-col gap-2 ${wrapperClass}`">
    <label
      :for="$.uid.toString()"
      :class="`${labelClass ? props.labelClass : 'text-surface-800 dark:text-surface-200 text-xs'}`"
      >{{ props.label }}</label
    >
    ...
    <div :class="`relative ${$attrs.disabled ? 'hover:cursor-not-allowed' : ''}`">
      ...
      <InputMask
        ...
        :class="`h-12 w-full ${$slots['prepend-icon'] ? 'pl-10' : ''} ${props.invalid ? '!ring-error-500 dark:!ring-error-300' : ''} ${$attrs.disabled ? '!text-surface-600 dark:!text-surface-0/70' : ''} `"
      />
    </div>
    <!-- End InputTex -->
